...
const InvoiceProcessor = ({ licenseIdsToLoad = [], onLoadComplete, caseNumberToLoad }: InvoiceProcessorProps) => {
    ...
    const loadLicensesToBasket = useCallback(
        async (licenseIds: string[]) => {
            try {
                setLoading(true);
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Always add a batch entry so the scan is visible in the results table
                const newResults = data.newCount > 0 ? data.scanResults.slice(-data.newCount) : [];
                const newBatch: FileBatch = {
        ...
        <>
            ...
            {showCaseModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fadeIn">
                    <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 animate-slideIn">
                        <h3 className="text-2xl font-bold text-gray-800 mb-4 font-hebrew text-center">הזן מספר תיק</h3>
            ...
            <div className="max-w-7xl mx-auto p-6 bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 animate-fadeIn">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-4 pb-2 border-b border-gray-100">
                    <div>
                        <h2 className="text-3xl font-bold text-gray-800 font-hebrew">מערכת הפקת רישיונות</h2>
                        <p className="text-gray-500 mt-1 font-hebrew">סריקת חשבוניות ופענוח נתונים (RTL Report)</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                        <div className="p-4 border-b border-gray-100 bg-gray-50/50">
                            <h3 className="font-bold text-gray-800 font-hebrew">דוח סריקה (Live Results)</h3>
                        </div>
                        ...
                        <div className="flex-1 overflow-auto">
                            <table className="w-full text-right" dir="rtl">
                                <thead className="bg-gray-50 sticky top-0 z-10 shadow-sm">
                                    <tr>
                                        <th className="px-4 py-3 text-sm font-bold text-gray-600 font-hebrew">
                                            מק"ט (ID)
                                        </th>
                                        <th className="px-4 py-3 text-sm font-bold text-gray-600 font-hebrew">
                                            שם תמרוק
                                            ...
                                            <React.Fragment key={batchIdx}>
                                                    ...
                                                    <td colSpan={5} className="px-4">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <span className="text-xs text-gray-500">
                                                                {batch.timestamp.toLocaleTimeString("he-IL", {
                                                                    hour: "2-digit",
                                                                    minute: "2-digit",
                                                                })}
                                                            </span>
                                                ...
                                                {batch.results.length === 0 ?
                                                    <tr className="bg-yellow-50 border-b border-yellow-100">
                                                        <td
                                                            colSpan={5}
                                                            className="px-4 py-3 text-sm text-center text-yellow-700 font-hebrew"
                                                        >
                                                            0 פריטים נוספו — הרישיונות כבר קיימים מסריקות קודמות
                                                        </td>
                                                    </tr>
                                                :   batch.results.map((row, idx) => (
                                                        <tr
                                                            key={`${batchIdx}-${row.id}-${idx}`}
                                                        ...
                                                        >
                                                            ...
                                                            <td className="px-4 py-3 text-sm text-gray-800 font-hebrew">
                                                                {row.productName}
                                                            </td>
                                                            {/* Clean License: No Slashes */}
                                                            <td className="px-4 py-3 text-sm font-mono text-gray-600">
                                                                {!/^[68]\d{7}$/.test(row.cleanLicense) ?
                                                                    row.cleanLicense
                                                                :   ""}
                                                            </td>
                                                            {/* Short Notification: Last 4 */}
                                                            <td className="px-4 py-3 text-sm font-mono text-gray-500 bg-gray-50/50">
                                                                {/^[68]\d{7}$/.test(row.cleanLicense) ?
                                                                    row.shortNotification
                                                                :   ""}
                                                            </td>
